...
export async function generateStaticParams() {
  try {
    const { posts } = await getPosts({ type: PostType.CARD, take: 100 });
...
export async function generateMetadata({
  ...
  try {
    ...
    if (!success || !post) return { title: '卡片未找到' };
    return {
      ...
      openGraph: {
        ...
        type: 'article',
        url: `${baseUrl}/cards/${params.slug}`,
        ...(post.coverImage ? { images: [{ url: post.coverImage }] } : {}),
      },
      alternates: { canonical: `${baseUrl}/cards/${params.slug}` },
    };
  } catch {
    return { title: '知识库卡片' };
  }
  ...
  return (
    <div className="min-h-screen py-12">
      <div style={{ 
        ...
        <Button variant="ghost" className="p-0 mb-8" href="/cards">
